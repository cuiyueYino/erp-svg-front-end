<template>
	<div>
		...
		<!--弹出框-->
		<el-dialog title="工作事项模板子表分类" center :visible.sync="dialogVisible" width="40%">
						...
						this.$refs.ruleFormTable.validate((valid) => {
							if(valid) {
								if(this.showFig) {
									...
									this.$api.collaborativeOffice.apiUrl("workItemTypeSub/insertWorkItemTypeSubModel", this.ruleForm).then(data => {
