...
let graph;
let guideIndex=0;
function init(id,width,height,data){
    ...
    G6.registerNode('node',{
        ...
        draw(item){
            const group=item.getGraphicGroup();
            let{name,status,animation}=item.getModel();
            ...
            if(animation&&animation==='flicker'){
                ...
                let c1=group.addShape('path',{
                    attrs:{
                        path:drawRect(0,0,60,30),
                        fill:'#47B2C4',
                        opacity:0.7,
                    }
                });
                let c2=group.addShape('path',{
                    attrs:{
                        path:drawRect(0,0,60,30),
                        fill:'#2AD77E',
                        opacity:0.7,
                ...
                c3.toBack();
                c1.animate({
                    path:drawRect(-10,-10,80,50),
                    opacity:0.1,
                    repeat:true
                },3000,'easeCubic',null,0);
                c2.animate({
                    path:drawRect(-10,-10,80,50),
                    opacity:0.1,
                    repeat:true
                },3000,'easeCubic',null,1000);
                c3.animate({
                    path:drawRect(-10,-10,80,50),
                    opacity:0.1,
                    repeat:true
                },3000,'easeCubic',null,2000)
            }
            return shape
        }
    });
    ...
    nodes=data.nodes.sort((a,b)=>{return a.y-b.y});minNode.y=nodes[0].y;
    ...
    G6.registerGuide('states',{
        draw(item){
            ...
            if(status!==undefined&&status.length>0){
                const group=item.getGraphicGroup();
                let x1=maxNode.x+100,
                y1=minNode.y+guideIndex*50+10;
                group.addShape('text',{
                    attrs:{
                        x:x1,
                        y:y1+10,
...
function update(data){
                    ...
                    draw(item){
                        ...
                        if(status!==undefined&&status.length>0){
                            ...
                            for(let i=0;i<status.length;i++){
                                if(i%2===0){
                                    group.addShape('circle',{
                                        attrs:{x:x1+10,y:y1+t1*50+25,r:5,text:status[i].name,fill:status[i].color,}
                                    });
                                    group.addShape('text',{attrs:{x:x1+20,y:y1+31+t1*50,text:status[i].name,fill:'black',}})
                                }else{
                                    group.addShape('circle',{attrs:{x:x1+90+status[i-1].name.length*4,y:y1+t1*50+25,r:5,text:status[i].name,fill:status[i].color,}});
                                    group.addShape('text',{attrs:{x:x1+100+status[i-1].name.length*4,y:y1+31+t1*50,text:status[i].name,fill:'black',}});
                                    t1++
                                }
                            }
                            guideIndex+=t1
                ...
                G6.registerGuide('legend',{
                    draw(item){
                        ...
                        if(label){
                            ...
                            label.forEach(item=>{
                                if(item.shape==='root'){
                                    group.addShape('text',{attrs:{x:x1+70,y:y1+32+guideIndex*50,text:item.name,fill:'black',}});
                                    root(group,x1,y1+10+guideIndex*50,15)
                                }else if(item.shape==='node'){
                                    group.addShape('text',{attrs:{x:x1+70,y:y1+32+guideIndex*50,text:item.name,fill:'black',}});
                                    node(group,x1,y1+10+guideIndex*50,'black')
    ...
    G6.registerGroup('lane',{
        draw(item){
            ...
            let{label}=item.getModel();
            const childrenBox=item.getChildrenBBox();
            group.addShape('text',{attrs:{x:childrenBox.x+(childrenBox.width-label.length*12)/2,y:minNode.y-30,text:label,fill:'black',}});
            group.addShape('rect',{attrs:{width:childrenBox.width+40,height:40,x:childrenBox.x-20,y:minNode.y-60,stroke:'black'}});
            return group.addShape('rect',{attrs:{width:childrenBox.width+40,height:maxNode.y-20,x:childrenBox.x-20,y:minNode.y-20,stroke:'black',}})
        }
    });
    graph.read(data)
}
    
function circleMath(r,x){return Math.sqrt(r*r-x*x)}
function getStatus(arr,value){
    ...
    for(let i=0;i<arr.length;i++){
        if(arr[i].title===value){re=arr[i];break}
    }
    if(JSON.stringify(re)==="{}"){re=arr[arr.length-1]}
    return re
}
...
function root(group,X,Y,radius){
    const width=60;
    const height=30;
    let path=[['M',width/2+X,Y],['L',width-radius+X,Y],];
    ...
    for(let i=-radius;i<=0;i++){
        let x,y;
        y=-circleMath(radius,i)+radius;x=i+radius;
        path.push(['L',x+X,y+Y])
    }
    path.push(['Z']);
    return group.addShape('path',{
        attrs:{path:path,fill:'#FFCCCC',stroke:'black'}
    })
}
