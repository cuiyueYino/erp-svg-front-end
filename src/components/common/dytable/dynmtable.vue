<template>
    <div>
        <div style="width:200px;height:40px;float:right;">
            <el-button style="margin-left:50px;magin-right:5px;" @click.prevent="addRow()" type="success" icon="el-icon-folder-add" size="medium">新增</el-button>
            <el-button  @click.prevent="delRowData()" type="danger" icon="el-icon-delete" size="medium">删除</el-button>
        </div>
        <div>
            <el-table
                :data="tableSData"
                    ...
                    <el-table-column 
                        ...
                        <template slot-scope="scope"   >
                            <el-input
                                size="mini"
                                placeholder="请输入内容"
                                @change="editInput(column,column.key,scope.row)"
                                v-model="scope.row[column.key]">
                            </el-input>
                        </template>
                    </el-table-column>
...
export default {
    ...
    methods:{
        ...
        //表格内输入框编辑触发
        editInput(column,key,row){
            let data = this.tableSData;
            this.$emit('Row-input',column,key,row,data);
        },
        //下拉框
        editselect(key,row){
